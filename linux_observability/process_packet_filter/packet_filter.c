...
// cgroup socket filter to filter packets based on process name and port
...
int block_process_ports(struct __sk_buff *skb) {
    ...
    // Get allowed process name from map
    __u32 key = 0;
    char *allowed_process_name = bpf_map_lookup_elem(&process_name_map, &key);
    if (!allowed_process_name) {
        return BPF_DROP;
    }

    // Check if the process name matches the allowed process name
    if (str_compare(program.program_name, allowed_process_name, sizeof(program.program_name)) == 0) {
        // Get allowed port from map
        __u32 *allowed_port = bpf_map_lookup_elem(&port_map, &key);
        if (!allowed_port) {
            return BPF_DROP;
        }

        // Allow traffic only on the allowed port
        if (th->dest != __bpf_htons(*allowed_port)) {
